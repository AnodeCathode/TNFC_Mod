...
public class BasicInventory implements IInventory {
	...
	// returns true if inventory changed
	public static boolean mergeStackIntoRange(IInventory source, IInventory dest, int inslot, int[] slots)
	{
		ItemStack input = source.getStackInSlot(inslot);
		if(input == null || input.stackSize == 0)
			return false;
	...
	public void readFromNBT(NBTTagList tag) {
        ...
        {
            NBTTagCompound item = (NBTTagCompound)tag.getCompoundTagAt(i);
